...
def render_occupancy_png_grouped(occ_df: pd.DataFrame, out_png: str,
    ...
    ax.set_xlabel("Residues (C12 block → MyT block)")

    # Bars (width will be auto-guarded below)
    if nA > 0:
        ax.bar(xA, dfA["Occupancy"].values, color="#1f77b4", width=bar_width, label="C12orf29")
    if nB > 0:
    ...
    if nA > 0 and nB > 0:
        ...
        ax.text(0.04, 1.02, "C12orf29", ha="left",  va="bottom", transform=ax.transAxes, color="#1f77b4",fontweight="bold")
...
def compose_stability_quad_from_pngs(
    ...
    panels = [
        ...
        (axs[1, 0], contacts_png, "C  Interface Contact Persistence (C12 ↔ MyT)"),
...
fig, ax = plt.subplots(figsize=(6, 4))
ax.plot(xA, rmsf_vals[idxA], lw=1.5, label="C12orf29", color="#1f77b4")
...
rmsf_msg = (f"[RMSF] C12 mean = {c12_mean:.2f} Å; "
...
with open(summary_path, "w") as f:
    f.write(
        f"C12–MyT stability analysis — {species_name} (AF3 → explicit-solvent MD)\n"
        f"Run date: {run_date}\n"
        "------------------------------------------------------\n"
        ...
        f"C12 kept (t0→tend): {c12_kept} / {c12_total} ({c12_kept_pct:.1f}%)   |   gained/lost: {c12_gained} / {c12_lost}\n"
